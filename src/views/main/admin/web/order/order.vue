<template>
  <div class="order-manage">
    <div class="search-form">
      <search-form :searchConfig="OrderSearchFormConfig" @resetSearchForm="resetSearchForm"
        @searchFormRequest="searchFormRequest" />
    </div>
    <div class="table-data">
      <table-form :tableData="defaultShowInfo" v-bind="OrderTableFormConfig" @handleOffsetChange="handleOffsetChange"
        @handlePageChange="handlePageChange">
        <!-- 表格插槽 -->
        <template #done="slotProps">
          <el-button class="btn-usable" plain size="mini" :type="slotProps.row.done ? 'primary' : 'info'"
            @click="handleStatus(slotProps.row)">{{ slotProps.row.done ? "进行中" : "已完成" }}</el-button>
        </template>
        <template #update_flag="slotProps">
          <el-button class="btn-usable" plain size="mini" :type="slotProps.row.update_flag ? 'primary' : 'info'"
            @click="handleUpdateFlag(slotProps.row)">{{ slotProps.row.update_flag ? "今日" : "往日" }}</el-button>
        </template>
